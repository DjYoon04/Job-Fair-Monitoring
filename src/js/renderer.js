...
async function exportTableToPdf(tableSelector, filename) {
  try {
    ...
    // If a month subtitle is present the header needs extra space to fit it.
    // Use `let` because we may override this value after rendering an HTML
    // snapshot of the header to an image (the image height determines the
    // true header height and must be reflected in table offsets).
    let headerH = monitoringMonthSubtitle ? 48 : 40; // mm

    // Optional cached header image generated from the HTML template. When
    // available we draw the rendered image for pixel-perfect fidelity.
    let cachedHeaderImage = null;
    let cachedHeaderImageAspect = 1;
    ...
    function drawHeader(doc, pageNum) {
      if (cachedHeaderImage) {
        try {
          ...
          const imgH = imgW / cachedHeaderImageAspect;
          doc.addImage(cachedHeaderImage, 'PNG', margin, y0, imgW, imgH);

          // Separator line below image
          const sepY = y0 + imgH + 2;
          ...
          doc.text('This document was automatically generated by the Job Fair Monitoring System', pageW / 2, pageH - 3.5, { align: 'center' });
        } catch (e) {
          // Fall back to text header if image draw fails
        }
        return;
      }
      const cx = pageW / 2;
      ...
      // Logos: slightly larger to match reference printout; vertically
      // centered within the header band.
      const logoH = 26;  // mm — larger seal size
      ...
      // "Republic of the Philippines" — small italic navy (slightly larger)
      ...
      // "Department of Migrant Workers" — bold, large to mimic the visual
      // prominence in the reference image.
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(20);
      ...
      // Document title — e.g. "2026 JOB FAIR MONITORING" (more prominent)
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(12);
      doc.setTextColor(27, 52, 87);
      ...
      // "FOR THE MONTH OF MARCH" — when applicable
    ...
    // If html2canvas is available, render the HTML header into a cached image
    // so the PDF header matches the on-screen header exactly (fonts, weights,
    // and logos). This is asynchronous but we await it before building the
    // autoTable so the image is available in didDrawPage.
    async function prepareHeaderImage() {
      if (typeof html2canvas === 'undefined') return null;
      try {
        // Build an offscreen wrapper with the HTML header
        ...
        wrapper.innerHTML = createPdfHeader(title, monitoringMonthSubtitle, 1);
        document.body.appendChild(wrapper);

        const canvas = await html2canvas(wrapper, { scale: 2, useCORS: true, backgroundColor: '#ffffff' });
        cachedHeaderImage = canvas.toDataURL('image/png');
        cachedHeaderImageAspect = canvas.width / canvas.height; // px aspect
        wrapper.remove();
      } catch (e) {
        // Ignore failures and fall back to textual header
    ...
    // Prepare header image (if possible) so the letterhead can be drawn as
    // an image for exact visual parity with the on-screen header. After
    // the image is ready, recompute `headerH` to match the rendered image
    // height so the table starts below the image without overlap/gaps.
    await prepareHeaderImage();
    if (cachedHeaderImage && cachedHeaderImageAspect > 0) {
      // imgW (mm) will be contentW; compute height in mm from aspect ratio.
      const imgH_mm = contentW / cachedHeaderImageAspect;
      // Add a small gap for the separator and breathing room.
      headerH = imgH_mm + 4; // mm
    }
